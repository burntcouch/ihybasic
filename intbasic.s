...
;
; **** converted to ca65 format and port to Hyrda-16                            *
; **** 2026 by Patrick Struthers                                                *
; ****
;********************************************************************************
.setcpu "65C02"
.debuginfo
...
;
;  jumpy places in Apple ROM, find Hydra-16 equivalents
;
;KBD            = $c000             ;R last key pressed + 128  ---  see BIOS section at end of this file
KBDSTRB         = $c010             ;RW keyboard strobe  --- PGS:  lookup

MON_NXTCHAR     = $fd75             ; PGS:  lookup this monitor routine
;MON_COUT       = $F8B6             ;print Acc to output device via $36-37 ---  see BIOS section at end of this file
;MONRDKEY       = $F843 
WRITE_CHAR      = $F8B6             ; will point to WRITE_CHAR in hydra bios
READ_CHAR       = $F843             ;  READ_CHAR
...
LE0ED:          jsr     LE00C
                ...
                beq     LE099
                cmp     #'('
                bne     BigLoop      ; not sure about this
                beq     LE099             ;(always)
...
LE350:           dey
                ...
                bpl     LE365             ;no, branch
Loop16:           lda     AUTOLN+1,x        ;AUTOLN = AUTOLN + AUTOINC
                adc     AUTOINC+1,x
                sta     AUTOLN+1,x
                inx
                beq     Loop16
...
Loop19:         jsr     MON_COUT          ;print error message
                iny
; 
...
LE470:           beq     Rtn7
                ...
                dex
                bpl     LE47D
                ldy     #<ErrMsg04        ;"TOO LONG"  (was ErrMsg01)
...
LE491:           asl     A                 ;double
                tay
                lda     #>SYNTABL/2     ;[#>SYNTABL/2] was SYNTABL-$7600
                rol     A
                sta     SYNPAG+1
LE498:           bne     LE49B
                iny
...
Loop38:         bne     LE7AE             ;no, adjust CH [always?]
                iny                       ;yes, go to next tab posn [sets Y-reg=1]
                bne     LE7B1             ;(always)

unref_e7bc:     bcs     Loop38
                rts
...
; 
ErrMsg00:        .byte   ">32767"
ErrMsg01:        .byte   "TOO LONG"
ErrMsg02:        .byte   "SYNTAX"
ErrMsg03:        .byte   "MEM FULL"
ErrMsg04:        .byte   "TOO MANY PARENS"
ErrMsg05:        .byte   "STRING"
ErrMsg06:        .byte   "NO END"
ErrMsg07:        .byte   "BAD BRANCH"
ErrMsg08:        .byte   "16 GOSUBS"
ErrMsg09:        .byte   "BAD RETURN"
ErrMsg10:        .byte   "16 FORS"
ErrMsg11:        .byte   "BAD NEXT"
ErrMsg12:        .byte   "STOPPED AT "
ErrMsg13:        .byte   "*** "
ErrMsg14:        .byte   " ERR"
                 .byte   $8d
ErrMsg15:        .byte   ">255"
ErrMsg16:        .byte   "RANGE"
ErrMsg17:        .byte   "DIM"
ErrMsg18:        .byte   "STR OVFL"
                 .byte   '\',$8d
ErrMsg20:        .byte    "RETYPE LINE"
                 .byte   $8d
ErrMsg21:        .byte   '?'
...
LEBFA:           jmp     TE801
                ...
                .byte  $ff,$ff,$ff,$ff      ; adjust 4 bytes to align to page boundary?
; 
; Token / syntax table.
; 
...
COLD:           ldy     #$00
                ...
                sta     LOMEM+1
                sta     HIMEM+1
Loop50:          inc     HIMEM+1           ;find top of RAM
                lda     (HIMEM),y
                eor     #$ff
                sta     (HIMEM),y
                cmp     (HIMEM),y
                ...
                sta     (HIMEM),y
                cmp     (HIMEM),y
                beq     Loop50             ;(always, unless RAM is broken)
...
; 
                dex                       ;X-reg = $ff
Loop52:          lda     PP+1,x            ;P3 = PP
                sta     P3+1,x
                sbc     AUX+1,x           ;P2 = PP - AUX
                sta     P2+1,x
                inx
                beq     Loop52           ; ? ;compare PV to P2
                bcc     JmpMEMFULL
                lda     PV
                cmp     P2
                lda     PV+1
...
VLOMEM:         jsr     GET16BIT
                ldy     ACC
                cpy     PP
; BUG: above line should be CPY PP, was cpy #PP
                lda     ACC+1
                sbc     PP+1
...
LOAD:           stx     XSAVE
                ...
                ldx     #$ff
                sec
Loop55:         lda     HIMEM+1,x         ;AUX = HIMEM - ACC
                sbc     ACC+1,x
                sta     AUX+1,x
                inx
                beq     Loop55
...
LF223:           inx
                inx
                lda     NOUNSTKC-1,x
                beq     Rtn23    ; may be Rtn22?  offset is 31 bytes in disassem, not 22
                jmp     LF3E0
...
Loop67:         lda     STK_40+1,y
                ...
                lda     STK_B0+1,y
                sta     STK_B0,y
; BUG: above two lines should be:  FIXED
;   LDA STK_B0+1,Y   was lda     STK_A0+1,y
;   STA STK_B0,Y     was sta     STK_A0,y
                iny
                cpy     FORNDX
                bcc     Loop67
Rtn40:          rts
...
LF380:          sta     SRCH+1            ;SRCH = ACC
                ...
                sty     NOUNSTKC,x
; 
Loop69:         lda     SRCH              ;srch = SRCH - 1
                bne     NoDec
                dec     SRCH+1            ;is SRCH negative?
                bmi     Rtn25             ;yes, return      which?
NoDec:          dec     SRCH
                lda     SRCH2
                ldy     #$00
                jsr     LE708
